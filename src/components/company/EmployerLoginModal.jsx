...
export default function EmployerLoginModal({ isOpen, onClose }) {
    ...
    const { sendOtp, verifyOtp, employerLogin, login, googleLoginEmployer, setVerifiedMobile } = useAuth();
    ...
    async function handleSendOtp(e) {
        ...
        try {
            ...
            setCountdown(30);
            setTimerActive(true);
            setInfoMessage(res.otpCode ? `Demo OTP code: ${res.otpCode} (or enter 123456)` : 'OTP code sent via SMS');
        } catch (err) {
    ...
    async function handleVerifyOtp(e) {
        ...
        try {
            ...
            if (res && res.exists) {
                // Already logged in directly via OTP match
                onClose();
                navigate('/company');
            } else {
                // Show choice: Create Account OR Already Have Account
                setStep('CHOICE');
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#0f172a' }}>
                            <span style={{ color: '#7c3aed' }}>Employer</span> Portal
                        </h3>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div style={{ padding: '28px' }}>
                    ...
                    {step === 'MOBILE' && (
                        <div>
                            ...
                            <form onSubmit={handleSendOtp}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 8px 16px rgba(124, 58, 237, 0.25)',
                            ...
                            >
                                <FiMail style={{ color: '#7c3aed' }} /> Already registered? Log in with Password / Google
                            </button>
                        </div>
                    )}
                                    ...
                                    {otp.map((data, index) => (
                                        <input
                                            ...
                                            style={{
                                                ...
                                                border: data ? '2px solid #7c3aed' : '1.5px solid #cbd5e1',
                                                textAlign: 'center', fontSize: '1.4rem', fontWeight: 800,
                                                color: '#1e293b', background: data ? '#f5f3ff' : '#f8fafc',
                                                outline: 'none'
                                            }}
                                        />
                                    ))}
                    ...
                    {step === 'CHOICE' && (
                        <div>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#f5f3ff', border: '1px solid #ddd6fe', color: '#6d28d9', padding: '5px 12px', borderRadius: '20px', fontSize: '0.82rem', fontWeight: 700, marginBottom: '14px' }}>
                            ...
                            <p style={{ color: '#64748b', fontSize: '0.88rem', marginBottom: '22px' }}>
                                Choose an option to access your employer services:
                            </p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        cursor: 'pointer', boxShadow: '0 8px 20px rgba(124, 58, 237, 0.25)',
                                        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                                        transition: 'all 0.2s'
                                    }}
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ fontSize: '0.82rem', opacity: 0.9 }}>New employer? Set up company profile & post jobs</div>
                                        </div>
                                    </div>
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                                        <div style={{ width: '42px', height: '42px', borderRadius: '12px', background: '#f5f3ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <FiLogIn size={22} color="#7c3aed" />
                                        </div>
                                        <div>
                    ...
                    {step === 'LOGIN' && (
                        <div>
                            {mobileNumber && (
                                <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: '#f5f3ff', border: '1px solid #ddd6fe', color: '#6d28d9', padding: '4px 10px', borderRadius: '16px', fontSize: '0.78rem', fontWeight: 700, marginBottom: '12px' }}>
                            ...
                            <form onSubmit={handleEmailLogin}>
                                ...
                                >
                                    <FiLogIn size={18} /> {loading ? 'Logging in...' : 'Login to Employer Dashboard'}
                                </button>
                            </form>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', textAlign: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '14px' }}>
                                <button
                                    type="button"
                                    onClick={handleGoToCreateAccount}
                                    style={{ background: 'none', border: 'none', color: '#7c3aed', fontWeight: 800, cursor: 'pointer', fontSize: '0.88rem' }}
                                >
